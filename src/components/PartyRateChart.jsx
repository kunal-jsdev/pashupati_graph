  ...
  return (
    <div className="relative" ref={ref}>
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-sm font-medium text-gray-700 shadow-sm hover:border-gray-400 focus:outline-none"
      >
        {current?.NAME ?? 'Select Grade'}
        <span className="text-gray-400">▾</span>
      </button>
      {open && (
        <div className="absolute right-0 z-20 mt-1 max-h-60 w-44 overflow-auto rounded-xl border border-gray-200 bg-white p-1 shadow-lg [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
          {options.map((opt) => (
            <button
              key={opt.CODE}
              type="button"
...
export default function PartyRateChart() {
  ...
  const momBadgeStyle = useMemo(() => {
    if (momShift === null) return ''
    if (momShift > 0) return 'bg-[#e6f4ea] text-[#137333] border border-[#ceead6]'
    if (momShift < 0) return 'bg-[#fce8e6] text-[#c5221f] border border-[#fad2cf]'
